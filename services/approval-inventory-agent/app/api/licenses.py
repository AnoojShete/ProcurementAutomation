...
async def mark_license_reviewed(
    ...
    """Mark a license as reviewed by admin, logging to reclaim history and setting 30-day cooldown."""
    reviewer = "admin@example.com"
    try:
        user = await get_current_user(request)
        if user:
            reviewer = user.email if hasattr(user, "email") else (user.get("email") if isinstance(user, dict) else str(user))
    except Exception:
        pass

    result = await UsageService.mark_reviewed(db, license_id, reviewer=reviewer)
    if result is None:
        raise HTTPException(status_code=404, detail=f"License {license_id} not found")
