...
class UsageAnomalyScorer:
    ...
    def score(
        ...
        """
        # No history is "insufficient_history" whether or not the model is
        # loaded — checked first so an untrained model doesn't report 0.0
        # ("normal") for a license nobody has ever logged into.
        if not sso_events:
            return {
                "anomaly_score": None,
                "model_version": "not_trained" if not self._load() else self.__class__._bundle.get("version", "unknown"),
                "top_factors": [],
