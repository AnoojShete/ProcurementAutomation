...
def test_full_procurement_flow():
    ...
    assert resp.status_code in [200, 201, 202], "Failed to upload document"

    print("3. Creating purchase request...")
    resp = requests.post(f"{BASE_URL}/requests/", json={
        "request_type": "hardware",
        "department": "Engineering",
        # Manager tier (single approver) under the default business rules;
        # 15000 now lands in manager+finance and would need two approvals.
        "amount": 2500,
        "currency": "INR"
    }, headers=req_headers)
